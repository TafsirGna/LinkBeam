...
export default class RelationshipsChart extends React.Component{
  ...
  setData(callback){
    ...
    var suggestedList = [], profilesList = [];

    for (var profile of this.props.objects){

      var profileName = dbDataSanitizer.fullName(profile.fullName);
      profileName = profileName.replaceAll(" ", "_").replaceAll("'", "_").replaceAll(",", "_");
      profilesList.push({
        name: profileName,
        size: Math.floor(Math.random() * 80) + 1,
        imports: [],
      });
      ...
      if (suggestions){
        ...
        for (var suggestion of suggestions){
          ...
          childrenList.push({
            name: suggestedName,
            size: Math.floor(Math.random() * 80) + 1,
            imports: ["suggestions.profiles." + profileName],
          });
